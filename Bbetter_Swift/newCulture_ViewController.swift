//
//  newCultureViewController.swift
//  Bbetter_Swift
//
...
class newCultureViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = table.dequeueReusableCell(withIdentifier: "cell", for: indexPath ) as! News_TableViewCell
               // let target = newsTitle[indexPath.row]
                ...
                do{
                    cell.culTitle?.text = try "\(newsTitle[indexPath.row].text())"
                    cell.culTime?.text = try "\(newsTime[indexPath.row].text())"
                }catch{}
    ...
    func crawl(){
                ...
                let url = URL(string: "https://news.daum.net/culture#1")

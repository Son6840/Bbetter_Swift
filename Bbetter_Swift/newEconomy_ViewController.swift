//
//  newEconomyViewController.swift
//  Bbetter_Swift
//
...
class newEconomyViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = table.dequeueReusableCell(withIdentifier: "cell", for: indexPath ) as! News_TableViewCell
               // let target = newsTitle[indexPath.row]
                ...
                do{
                    cell.ecoTitle?.text = try "\(newsTitle[indexPath.row].text())"
                    cell.ecoTime?.text = try "\(newsTime[indexPath.row].text())"
                }catch{}
                return cell
    ...
    func crawl(){
                ...
                let url = URL(string: "https://news.daum.net/economic#1")
